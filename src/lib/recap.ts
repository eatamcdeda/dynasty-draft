import { playerRank, slotLabel } from "./types";
import type { SnakeDraftEngine } from "./draft";
import type { Position } from "./types";

export const POS_ORDER: Position[] = ["QB", "RB", "WR", "TE", "DL", "LB", "DB", "DST"];
...
export function summarizeDraft(engine: SnakeDraftEngine) {
  const scoring = engine.settings.scoring;
  const userPicks = engine.picks.filter((pick) => pick.teamIndex === engine.userTeamIndex);
  const counts = Object.fromEntries(POS_ORDER.map((pos) => [pos, 0])) as Record<Position, number>;
  const rows: RecapRow[] = userPicks.map((pick) => {
...
function qualityFromDelta(delta: number) {
...
function letterFromScore(score: number) {
...
function conicFromCounts(counts: Record<Position, number>, total: number) {
  let deg = 0;
  const parts: string[] = [];
...
function pieSlices(counts: Record<Position, number>, total: number) {
  const slices: Array<{ pos: Position; count: number; color: string; d: string }> = [];
  let angle = -90;
  for (const pos of POS_ORDER) {
    const n = counts[pos] ?? 0;
    if (!n) continue;
...
export function barWidth(delta: number) {
